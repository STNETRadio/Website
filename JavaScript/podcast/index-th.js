document.addEventListener("DOMContentLoaded", () => {
    ...
    const podcasts = [
        ...
        {
            title: "Talking with LITALK", creator: "LITALK Education", image: "https://s3.ap-southeast-1.amazonaws.com/rss.stnetradio.com/SoaqerStudio/LITALKPodcast/Logo.png", rating: "รายการเร็วๆ นี้", listeners: "1K", link: "https://podcasts.apple.com/th/podcast/talking-with-litalk/id1786381161",
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                <div class="pwa-modal">
                    ...
                    <h3>สำหรับ PWA เท่านั้น</h3>
                    <p>ฟีเจอร์เครื่องเล่นนี้ออกแบบมาเพื่อประสบการณ์ที่ดีที่สุดบนแอปพลิเคชัน<br>กรุณาเพิ่มเว็บไซต์นี้ลงในหน้าจอโฮมเพื่อใช้งาน</p>
                    <button class="pwa-modal-btn" onclick="closePWAPrompt()">ตกลง</button>
                </div>
            `;
    ...
    function loadFeed(feedUrl) {
        ...
        fetch(proxiedUrl, { signal: controller.signal })
            ...
            .then(data => {
                ...
                if (items.length === 0) {
                    episodeListContainer.innerHTML = '<div style="text-align:center; padding:2rem;">ไม่พบตอน</div>';
                } else {
                            ...
                            card.innerHTML = `
                                ...
                                <div class="ep-meta">
                                    <div class="play-btn-small">
                                        ${(currentEpisode && currentEpisode.url === audioUrl && isPlaying) ? '<i class="fa-solid fa-pause"></i> กำลังเล่น' : '<i class="fa-solid fa-play"></i> เล่น'}
                                    </div>
                                </div>
                            `;
            ...
            .catch(err => {
                console.error(err);
                loadingScreen.innerHTML = `<div style="text-align:center; padding:20px;">เกิดข้อผิดพลาดในการโหลด:<br>${err.message}<br><br>กรุณาลองใหม่ภายหลัง</div>`;
            });
    }
    ...
    mainAudio.addEventListener('error', (e) => {
        ...
        alert("ไม่สามารถเล่นตอนนี้ได้ ไฟล์เสียงอาจมีปัญหา");
        isPlaying = false;
        updatePlayIcons(false);
    });
